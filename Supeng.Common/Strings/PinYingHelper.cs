...
namespace Supeng.Common.Strings
{
  public static class PinYingHelper
  {
    ...
    private static readonly string[] AllValue =
    {
      "zuo", "zun", "zui", "zuan", "zu",
    ...
    private static readonly int[] AllCode =
    {
    ...
    public static char GetFirstPy(char ch)
    {
      ...
      int gb = (bytes[0] << 8 & 0xff00) + (bytes[1] & 0xff);
      if (gb < FirstTable[0])
        return ch;
      int i;
      for (i = 0; i < 26; ++i)
    ...
    {
      byte[] bytes = Encoding.GetEncoding("gb2312").GetBytes(new string(cn, 1));
    ...
    private static string GetSpellByAscii(int ascii)
    {
      ...
      int first = 0;
      int sLast = AllCode.Length - 1;
      int last = AllCode.Length - 1;
      while (true)
      {
        int mid = (first + last) >> 1;
        if (ascii == AllCode[mid])
        {
          return AllValue[mid];
        }
        if (ascii > AllCode[mid])
    ...
    private static bool Match(int i, int gb)
    {
      ...
      int j = i + 1;
      // 字母Z使用了两个标签
      while (j < 26 && (FirstTable[j] == FirstTable[i]))
      {
        ++j;
      }
      if (j == 26)
        return gb <= FirstTable[j];
      return gb < FirstTable[j];
    }
